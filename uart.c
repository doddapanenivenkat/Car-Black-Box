...
void putch(unsigned char byte) {
    ...
    while (!TXIF) //whenever TXIF = 1 -> Empty, TXIF = 0 -> Full
    {
        continue;
    }
    TXIF = 0;
    TXREG = byte; //reading the data(byte) to buffer register(TXREG)
}

int puts(const char *s) {
    while (*s) {
        putch(*s++); //byte by byte to transmit
    }
    return 0;
}
...
unsigned char getch(void) {
    ...
    while (!RCIF) //whenever RCIF = 0 -> Empty, RCIF = 1 -> Full
    {
        continue;
    }
    RCIF = 0;
    return RCREG; //read the data from buffer register (one byte)  
}

unsigned char getche(void) {
    /* Receive 1 byte of data and transmit the same data*/
    unsigned char c;
